...
function displayDetailPokeMonInformation(pokeMonsName){	
	for(var pokeMon = 0;pokeMon < pokeMonDatas.pokemon.length;pokeMon++)
	{		
		var arrayData = pokeMonDatas.pokemon[pokeMon]
		if(arrayData.name == pokeMonsName)
		{
			for(var item in arrayData)
			{
				alert(item+":"+arrayData[item])
			}	
			break
		}	
	}
...
function displayPokemanInfoWithNextEvolution(nextEvolutionName){
	for(var pokeMon = 0;pokeMon < pokeMonDatas.pokemon.length;pokeMon++)
	{
		var nextEvolution = pokeMonDatas.pokemon[pokeMon]["next_evolution"]
		for(var item in nextEvolution)
			if(nextEvolution[item]["name"] == nextEvolutionName)
				alert(pokeMonDatas.pokemon[pokeMon]["name"])
	}	
}
...
//	the dataset)
function displayPokemanInfoWithWeaknesses(weaknessName){
	for(var pokeMon = 0;pokeMon < pokeMonDatas.pokemon.length;pokeMon++)
	{
		var weakNess = pokeMonDatas.pokemon[pokeMon]['weaknesses']
		for(var item in weakNess)
			if(weakNess[item] == weaknessName)		
				alert(pokeMonDatas.pokemon[pokeMon]["name"])
	}
}

//displayDetailPokeMonInformation(window.prompt("Enter Pokemon's Name"))
//displayPokemanInfoWithNextEvolution(window.prompt("Enter Pokeman's next_evolution Name:"))
//displayPokemanInfoWithWeaknesses(window.prompt("Enter Pokeman's weakness:"))
